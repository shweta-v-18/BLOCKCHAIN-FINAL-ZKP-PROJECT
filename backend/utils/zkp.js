...
const circuitWasmPath = path.join(__dirname, "../../zkp/circuit.wasm")
const circuitZkeyPath = path.join(__dirname, "../../zkp/circuit_final.zkey")
...
export async function generateProof(privateData, salt) {
  try {
    ...
    const input = {
      commitment: BigInt("0x" + commitment.substring(0, 16)),
      salt: BigInt("0x" + salt.substring(0, 16)),
    }
...
export async function verifyProof(proof, expectedCommitment) {
  try {
...
function simulateProof(privateData, salt) {
  const commitment = createCommitment(privateData, salt)
  return {
    proof: {
